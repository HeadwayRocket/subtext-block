(function($){
	...
	$(document).ready(function($){
    	...
    	fieldActivation = function(selector) {
    	
    		if ( $(selector).is(":checked") )
    			$(selector).next().removeClass('deactivated');
    		else
    			$(selector).next().addClass('deactivated');
    	
    	}
    	
    	/* we initiate the admin selects */
    	$('.btr-options .select-field, .btr-options .multiselect-field').select2();
    	
    	/* we deactivated the fields if necessaray */
    	$('.btr-options .checkbox-field.activation').each( function() {
    		
    		$(this).click( function() {
    			fieldActivation($(this));
    		});
    		
    		fieldActivation($(this));
    	});
    	
    	/* we add the javascript for the read more */    	
    	$('.btr-extended-content').hide();
    	
    	$('.btr-read-more').show().click( function(e) {
    	...
    	/* we save the form via ajax */
		$('.btr-options-form').submit( function(e) {
		
			e.preventDefault();
			
			var formData = $(this).formDataToJson();
			
			$.ajax({
				type: 'POST',
				url: ajaxurl,
				dataType: "json",
				data: { entries: formData, action: 'butler_admin_save_options'},
				beforeSend: function() {
					
					$('.btr-options-form').find('input[type=submit]')
						.addClass('button-primary-disabled')
						.parent().append('<span class="spinner"></span>');
